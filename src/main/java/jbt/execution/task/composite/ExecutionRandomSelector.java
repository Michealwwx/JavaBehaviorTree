...
public class ExecutionRandomSelector extends ExecutionComposite {
	 ...
	 * {@link ExecutionTask.Status#RUNNING} is returned. If it has successfully finished, it
	 * returns {@link ExecutionTask.Status#SUCCESS}. If it has failed, it tries to spawn the
	 * next child (returning {@link ExecutionTask.Status#RUNNING}). If it was the last child,
	 * returns {@link ExecutionTask.Status#FAILURE}.
	 * 
	 * @see ExecutionTask#internalTick()
	 */
	protected ExecutionTask.Status internalTick() {
		ExecutionTask.Status childStatus = this.activeChild.getStatus();

		if (childStatus == ExecutionTask.Status.RUNNING) {
			return ExecutionTask.Status.RUNNING;
		}
		else if (childStatus == ExecutionTask.Status.SUCCESS) {
			return ExecutionTask.Status.SUCCESS;
		}
		else {
	 ...
	 * 
	 * @see ExecutionTask#statusChanged(TaskEvent)
	 */
