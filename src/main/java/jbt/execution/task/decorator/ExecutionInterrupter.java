 ...
 * {@link #interrupt(ExecutionTask.Status)} must be called.
 * 
...
public class ExecutionInterrupter extends ExecutionDecorator {
	 ...
	 * interrupted.
	 */
	private ExecutionTask.Status statusSet;
	/**
	 ...
	 * was passed to the {@link #interrupt(ExecutionTask.Status)} method. Otherwise, returns
	...
	protected ExecutionTask.Status internalTick() {
		if (this.interrupted) {
			...
			ExecutionTask.Status childStatus = this.executionChild.getStatus();
	 ...
	 */
	public void interrupt(ExecutionTask.Status status) {
		if (!this.interrupted) {
			/*
			...
			if (!this.getTerminated()) {
				if (status != ExecutionTask.Status.FAILURE && status != ExecutionTask.Status.SUCCESS) {
